...
angular.module('xgames').factory('LeagueFactory', function($firebaseArray, $q, defaultConfig, $localstorage) {
    var LeagueFactory = $firebaseArray.$extend({
        getLeaguesAsync: function () {
            var deferred = $q.defer();
            this.$loaded(function (data) {
                deferred.resolve(data);
            });
    ...
    return function () {
        var ref = new Firebase(defaultConfig.firebaseBackend + '/leagues');
        return new LeagueFactory(ref);
    };
});
